...
def coordonnees_afficher(order_by,id_coordonnees_sel):
    ...
    if request.method == "GET":
        try:
            ...
            obj_actions_coordonnees = GestionGenres()
            # Récupère les données grâce à une requête MySql définie dans la classe GestionGenres()
            # Fichier data_gestion_coordonnees.py
            # "order_by" permet de choisir l'ordre d'affichage des genres.
            data_coordonnees = obj_actions_coordonnees.coordonnees_afficher_data(order_by,id_coordonnees_sel)
            # DEBUG bon marché : Pour afficher un message dans la console.
            print(" data genres", data_coordonnees, "type ", type(data_coordonnees))

            # Différencier les messages si la table est vide.
            if not data_coordonnees and id_coordonnees_sel == 0:
                flash("""La table "coordonnees" est vide. !!""", "warning")
            elif not data_coordonnees and id_coordonnees_sel > 0:
    ...
    return render_template("coordonnees/coordonnees_afficher.html", data=data_coordonnees)


# ---------------------------------------------------------------------------------------------------
# OM 2020.04.07 Définition d'une "route" /coordonnees_add ,
# cela va permettre de programmer quelles actions sont réalisées avant de l'envoyer
# au navigateur par la méthode "render_template"
# En cas d'erreur on affiche à nouveau la page "coordonnees_add.html"
# Pour la tester http://127.0.0.1:5005/coordonnees_add
# ---------------------------------------------------------------------------------------------------
@obj_mon_application.route("/coordonnees_add", methods=['GET', 'POST'])
def coordonnees_add ():
    ...
    if request.method == "POST":
        try:
            # OM 2020.04.09 Objet contenant toutes les méthodes pour gérer (CRUD) les données.
            obj_actions_coordonnees = GestionGenres()
            # OM 2020.04.09 Récupère le contenu du champ dans le formulaire HTML "coordonnees_add.html"
            name_coordonnees = request.form['name_coordonnees_html']
            ...
            if not re.match("^([A-Z]|[a-zÀ-ÖØ-öø-ÿ])[A-Za-zÀ-ÖØ-öø-ÿ]*['\- ]?[A-Za-zÀ-ÖØ-öø-ÿ]+$",
                ...
                # On doit afficher à nouveau le formulaire "coordonnees_add.html" à cause des erreurs de "claviotage"
                return render_template("coordonnees/coordonnees_add.html")
            else:

                # Constitution d'un dictionnaire et insertion dans la BD
                valeurs_insertion_dictionnaire = {"value_mail": name_coordonnees}
                obj_actions_coordonnees.add_coordonnees_data(valeurs_insertion_dictionnaire)
...
# ---------------------------------------------------------------------------------------------------
# OM 2020.04.07 Définition d'une "route" /coordonnees_edit ,
# cela va permettre de programmer quelles actions sont réalisées avant de l'envoyer
# au navigateur par la méthode "render_template".
# On change la valeur d'un genre de films par la commande MySql "UPDATE"
# ---------------------------------------------------------------------------------------------------
@obj_mon_application.route('/coordonnees_edit', methods=['POST', 'GET'])
def coordonnees_edit ():
    ...
    if request.method == 'GET':
        try:
            ...
            # <a href="{{ url_for('coordonnees_edit', id_coordonnees_edit_html=row.id_coordonnees) }}">Edit</a>
            ...
            data_id_coordonnees = obj_actions_coordonnees.edit_coordonnees_data(valeur_select_dictionnaire)
    ...
    return render_template("coordonnees/coordonnees_edit.html", data=data_id_coordonnees)


# ---------------------------------------------------------------------------------------------------
# OM 2020.04.07 Définition d'une "route" /coordonnees_update , cela va permettre de programmer quelles actions sont réalisées avant de l'envoyer
# au navigateur par la méthode "render_template".
# On change la valeur d'un genre de films par la commande MySql "UPDATE"
# ---------------------------------------------------------------------------------------------------
@obj_mon_application.route('/coordonnees_update', methods=['POST', 'GET'])
def coordonnees_update ():
    ...
    if request.method == 'POST':
        try:
            ...
            # Récupère la valeur de "id_coordonnees" du formulaire html "coordonnees_edit.html"
            # l'utilisateur clique sur le lien "edit" et on récupère la valeur de "id_coordonnees"
            # grâce à la variable "id_coordonnees_edit_html"
            # <a href="{{ url_for('coordonnees_edit', id_coordonnees_edit_html=row.id_coordonnees) }}">Edit</a>
            id_coordonnees_edit = request.values['id_coordonnees_edit_html']
            ...
            name_coordonnees = request.values['name_edit_mail_html']
            valeur_edit_list = [{'id_coordonnees': id_coordonnees_edit, 'mail': name_coordonnees}]
            # On ne doit pas accepter des valeurs vides, des valeurs avec des chiffres,
            # des valeurs avec des caractères qui ne sont pas des lettres.
            # Pour comprendre [A-Za-zÀ-ÖØ-öø-ÿ] il faut se reporter à la table ASCII https://www.ascii-code.com/
            # Accepte le trait d'union ou l'apostrophe, et l'espace entre deux mots, mais pas plus d'une occurence.
            if not re.match("^([A-Z]|[a-zÀ-ÖØ-öø-ÿ])[A-Za-zÀ-ÖØ-öø-ÿ]*['\- ]?[A-Za-zÀ-ÖØ-öø-ÿ]+$",
                ...
                # On doit afficher à nouveau le formulaire "coordonnees_edit.html" à cause des erreurs de "claviotage"
                # Constitution d'une liste pour que le formulaire d'édition "coordonnees_edit.html" affiche à nouveau
                # la possibilité de modifier l'entrée
                # Exemple d'une liste : [{'id_coordonnees': 13, 'mail': 'philosophique'}]
                valeur_edit_list = [{'id_coordonnees': id_coordonnees_edit, 'mail': name_coordonnees}]

                # DEBUG bon marché :
                # Pour afficher le contenu et le type de valeurs passées au formulaire "coordonnees_edit.html"
                print(valeur_edit_list, "type ..", type(valeur_edit_list))
                return render_template('coordonnees/coordonnees_edit.html', data=valeur_edit_list)
            else:
                # Constitution d'un dictionnaire et insertion dans la BD
                valeur_update_dictionnaire = {"value_id_coordonnees": id_coordonnees_edit, "value_name_coordonnees": name_coordonnees}

                # OM 2020.04.09 Objet contenant toutes les méthodes pour gérer (CRUD) les données.
                obj_actions_coordonnees = GestionGenres()

                # La commande MySql est envoyée à la BD
                data_id_coordonnees = obj_actions_coordonnees.update_coordonnees_data(valeur_update_dictionnaire)
                # DEBUG bon marché :
                print("dataIdGenre ", data_id_coordonnees, "type ", type(data_id_coordonnees))
                # Message ci-après permettent de donner un sentiment rassurant aux utilisateurs.
...
# ---------------------------------------------------------------------------------------------------
# OM 2020.04.07 Définition d'une "route" /coordonnees_select_delete , cela va permettre de programmer quelles actions sont réalisées avant de l'envoyer
# au navigateur par la méthode "render_template".
# On change la valeur d'un genre de films par la commande MySql "UPDATE"
# ---------------------------------------------------------------------------------------------------
@obj_mon_application.route('/coordonnees_select_delete', methods=['POST', 'GET'])
def coordonnees_select_delete ():
    if request.method == 'GET':
        try:
            ...
            # OM 2019.04.02 La commande MySql est envoyée à la BD
            data_id_coordonnees, films_associes_coordonnees_delete  = obj_actions_coordonnees.delete_select_coordonnees_data(valeur_delete_dictionnaire)
            print("data_films_attribue_coordonnees_delete ",films_associes_coordonnees_delete," data_id_coordonnees ",data_id_coordonnees)
            ...
            # DEBUG bon marché : Pour afficher un message dans la console.
            print(f"Erreur coordonnees_delete {erreur.args[0], erreur.args[1]}")
            # C'est une erreur à signaler à l'utilisateur de cette application WEB.
            flash(f"Erreur coordonnees_delete {erreur.args[0], erreur.args[1]}", "danger")

    # Envoie la page "HTML" au serveur.
    return render_template('coordonnees/coordonnees_delete.html',
                           data=data_id_coordonnees,
                           data_films_associes = films_associes_coordonnees_delete)
...
# ---------------------------------------------------------------------------------------------------
@obj_mon_application.route('/coordonnees_delete', methods=['POST', 'GET'])
def coordonnees_delete ():
    ...
    if request.method == 'POST':
        try:
            ...
            # OM 2020.04.09 Objet contenant toutes les méthodes pour gérer (CRUD) les données.
            obj_actions_coordonnees = GestionGenres()
            # OM 2019.04.02 Récupère la valeur de "id_coordonnees" par une variable de session de "coordonnees_select_delete"
            id_coordonnees_delete = session['session_id_coordonnees_delete']
            print(" coordonnees_delete id_coordonnees_delete ", id_coordonnees_delete)
            # Constitution d'un dictionnaire et insertion dans la BD
            valeur_delete_dictionnaire = {"value_id_coordonnees": id_coordonnees_delete}

            data_coordonnees = obj_actions_coordonnees.delete_coordonnees_data(valeur_delete_dictionnaire)
