...
class GestionGenres():
    def __init__ (self):
        try:
            ...
            flash(f"Dans Gestion coordonnees ...terrible erreur, il faut connecter une base de donnée", "danger")
            # DEBUG bon marché : Pour afficher un message dans la console.
    ...
    def coordonnees_afficher_data (self, valeur_order_by, id_coordonnees_sel):
        try:
            ...
            with MaBaseDeDonnee().connexion_bd.cursor() as mc_afficher:
                # Afficher soit la liste des coordonnees dans l'ordre inverse ou simplement le genre sélectionné
                # par l'action edit
    ...
    def add_coordonnees_data (self, valeurs_insertion_dictionnaire):
        try:
            ...
            # OM 2020.04.07 C'EST LA QUE VOUS ALLEZ DEVOIR PLACER VOTRE PROPRE LOGIQUE MySql
            strsql_insert_coordonnees = """INSERT INTO coordonnees (id_coordonnees,mail) VALUES (NULL,%(value_mail)s)"""
            # Du fait de l'utilisation des "context managers" on accède au curseur grâce au "with".
    ...
    def edit_coordonnees_data (self, valeur_id_dictionnaire):
        try:
            ...
            print(f"Problème edit_coordonnees_data Data Gestions coordonnees numéro de l'erreur : {erreur}")
            # flash(f"Flash. Problèmes Data Gestions coordonnees numéro de l'erreur : {erreur}", "danger")
            # OM 2020.04.09 On dérive "Exception" par le "@obj_mon_application.errorhandler(404)" fichier "run_mon_app.py"
            # Ainsi on peut avoir un message d'erreur personnalisé.
            raise Exception(
                "Raise exception... Problème edit_coordonnees_data d'un genre Data Gestions coordonnees {erreur}")
    ...
    def update_coordonnees_data (self, valeur_update_dictionnaire):
        try:
            ...
            str_sql_update_intitulegenre = "UPDATE coordonnees SET mail = %(value_name_coordonnees)s WHERE id_coordonnees = %(value_id_coordonnees)s"
            ...
            with MaBaseDeDonnee().connexion_bd as mconn_bd:
                ...
                TypeError) as erreur:
            # OM 2020.03.01 Message en cas d'échec du bon déroulement des commandes ci-dessus.
            print(f"Problème update_coordonnees_data Data Gestions coordonnees numéro de l'erreur : {erreur}")
            # flash(f"Flash. Problèmes Data Gestions coordonnees numéro de l'erreur : {erreur}", "danger")
            # raise Exception('Raise exception... Problème update_coordonnees_data d\'un genre Data Gestions coordonnees {}'.format(str(erreur)))
    ...
    def delete_select_coordonnees_data (self, valeur_delete_dictionnaire):
        try:
            ...
            str_sql_coordonnees_films_delete = """SELECT id_coordonnees_film, nom_film, id_coordonnees, mail FROM coordonnees_films
                                            INNER JOIN t_films ON coordonnees_films.fk_film = t_films.id_film
                                            INNER JOIN coordonnees ON coordonnees_films.fk_coordonnees = coordonnees.id_coordonnees
            ...
            with MaBaseDeDonnee().connexion_bd as mconn_bd:
                ...
                TypeError) as erreur:
            # DEBUG bon marché : Pour afficher un message dans la console.
            print(f"Problème delete_select_coordonnees_data Gestions coordonnees numéro de l'erreur : {erreur}")
            # C'est une erreur à signaler à l'utilisateur de cette application WEB.
            flash(f"Flash. Problème delete_select_coordonnees_data numéro de l'erreur : {erreur}", "danger")
            raise Exception(
                "Raise exception... Problème delete_select_coordonnees_data d\'un genre Data Gestions coordonnees {erreur}")
    ...
    def delete_coordonnees_data (self, valeur_delete_dictionnaire):
        try:
            ...
            # le "%s" permet d'éviter des injections SQL "simples"
            # <td><input type = "text" name = "nameEditIntituleGenreHTML" value="{{ row.mail }}"/></td>
            str_sql_delete_films_coordonnees = """DELETE FROM coordonnees_films WHERE fk_coordonnees = %(value_id_coordonnees)s"""
            ...
            with MaBaseDeDonnee().connexion_bd as mconn_bd:
                ...
                TypeError) as erreur:
            # DEBUG bon marché : Pour afficher un message dans la console.
            print(f"Problème delete_coordonnees_data Data Gestions coordonnees numéro de l'erreur : {erreur}")
            # flash(f"Flash. Problèmes Data Gestions coordonnees numéro de l'erreur : {erreur}", "danger")
